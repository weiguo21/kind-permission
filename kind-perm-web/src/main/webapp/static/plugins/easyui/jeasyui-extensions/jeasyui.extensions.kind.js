...
function messageBox(result){
	if(result.status==200){
		 parent.$.messager.show({ title : "提示",msg:"操作成功！", position: "bottomRight" });
		 parent.d.panel('close');
	}else{
		 parent.$.messager.show({ title:"提示",msg:result.msg, position:"bottomRight" });
		 parent.d.panel('close');
	}
}
function messageTip(data,dg,d){
	...
	if(result.status==200){
		...
		parent.$.messager.alert(result.msg);
		return false;
	}  
}
